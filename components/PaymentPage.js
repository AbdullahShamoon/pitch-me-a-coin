...
const PaymentPage = (params) => {
    ...
    const pay = async (amount) => {
        //get the order id
        let a = await initiate(amount, params.p.username, paymentform)
        let orderId = a.id
        ...
        <>
            ...
            <div className="payment flex items-center justify-center w-full mb-20" id='Payment'>
                <div className="bg-white rounded-lg shadow-2xl p-9 w-[90%] ">
                    ...
                    <div className="mb-4">
                        <input onChange={handleChange} value={paymentform.name} name="name" type="text" className='border border-gray-400 rounded-lg py-1 px-2 w-full text-gray-800' id="name" placeholder="Enter your name" />
                        <input onChange={handleChange} value={paymentform.message} name="message" type="text" className='border border-gray-400 rounded-lg py-1 px-2 w-full my-3 text-gray-800' id="name" placeholder="Enter Message" />
                    ...
                    <div className="payButton mt-4">
                        <button onClick={() => pay(rangeval * 100)} className="relative  inline-flex items-center justify-start py-3 pl-4 pr-12 overflow-hidden font-semibold text-indigo-700 transition-all duration-150 ease-in-out rounded hover:pl-10 hover:pr-6 bg-gray-50 group disabled:line-through" disabled={paymentform.name?.length < 3 || paymentform.message?.length < 3}>
